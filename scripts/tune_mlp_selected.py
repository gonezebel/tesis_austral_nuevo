...
SUMMARY_GLOB = REPO_ROOT / "reports" / "10_modelo_redmlp"
OUTPUT_BASE = REPO_ROOT / "reports" / "12_tuning_redmlp"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Tuning MLP solo sobre la base/balanceo ganadores del resumen previo."
    )
    parser.add_argument("--summary-csv", type=Path, default=None, help="Ruta al resumen_modelos_redmlp.csv")
    ...
    parser.add_argument("--n-iter", type=int, default=18, help="Iteraciones de RandomizedSearchCV")
...
def find_latest_summary() -> Path:
    candidates = sorted(SUMMARY_GLOB.glob("**/resumen_modelos_redmlp.csv"), key=lambda p: p.stat().st_mtime)
    if not candidates:
        raise FileNotFoundError("No se encontro resumen_modelos_redmlp.csv en reports/10_modelo_redmlp")
    return candidates[-1]
...
def main() -> None:
    ...
    param_distributions = {
        ...
        "model__learning_rate_init": [0.0005, 0.001, 0.002, 0.004],
    ...
    output_dir = OUTPUT_BASE / run_id / f"RedMLP_tuning_{datetime.today().strftime('%Y-%m-%d')}"
    output_dir.mkdir(parents=True, exist_ok=True)

    cv_results = pd.DataFrame(search.cv_results_).sort_values("rank_test_recall_target")
    cv_results.to_csv(output_dir / "cv_results_redmlp_tuning.csv", index=False)
    ...
    pd.DataFrame([summary]).to_csv(output_dir / "resumen_tuning_redmlp.csv", index=False)

    with open(output_dir / "best_params_redmlp.json", "w", encoding="utf-8") as f:
        json.dump(search.best_params_, f, indent=2)

    report_test = pd.DataFrame(classification_report(y_test, y_pred_test, output_dict=True, zero_division=0)).T
    report_test.to_csv(output_dir / "classification_report_test_redmlp.csv")

    print("Resumen tuning RedMLP")
