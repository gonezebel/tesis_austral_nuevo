...
SUMMARY_GLOB = REPO_ROOT / "reports" / "09_modelo_lightgbm"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Tuning LightGBM solo sobre la base/balanceo ganadores del resumen previo."
    )
    parser.add_argument("--summary-csv", type=Path, default=None, help="Ruta al resumen_modelos_lightgbm.csv")
...
def find_latest_summary() -> Path:
    candidates = sorted(SUMMARY_GLOB.glob("**/resumen_modelos_lightgbm.csv"), key=lambda p: p.stat().st_mtime)
    if not candidates:
        raise FileNotFoundError("No se encontro resumen_modelos_lightgbm.csv en reports/09_modelo_lightgbm")
    return candidates[-1]
...
def build_pipeline(balancer, num_classes: int, random_state: int) -> Pipeline:
...
def main() -> None:
    ...
    pipeline = build_pipeline(balancer, num_classes, args.random_state)
